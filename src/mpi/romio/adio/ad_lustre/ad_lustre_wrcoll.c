...
typedef struct {
    int       count; /* number displacement-length pairs */
 ...
 *   IN: flat_ftype: this rank's flattened fileview datatype
 *       flat_ftype.count: number of noncontiguous requests in file datatype.
                              ...
                              ADIO_Offset **buf_idx)
{
    ...
    *my_req_ptr = my_req;
     ...
     */
    aggr_ranks = (int *) ADIOI_Malloc(flat_ftype.count * sizeof(int));
    avail_lens = (ADIO_Offset *) ADIOI_Malloc(flat_ftype.count * sizeof(ADIO_Offset));

    /* nelems will be the number of offset-length pairs for my_req[] */
    nelems = 0;
    for (i = 0; i < flat_ftype.count; i++) {
        /* short circuit offset/len processing if zero-byte read/write. */
        if (flat_ftype.lens[i] == 0)
            continue;

        off = flat_ftype.offsets[i];
        avail_len = flat_ftype.lens[i];
         ...
         * expensive for large value of flat_ftype.count, we keep a copy of
        ...
        avail_lens[i] = avail_len;     /* length covered, may be < flat_ftype.lens[i] */
#ifdef WKL_DEBUG
assert(aggr >= 0 && aggr <= cb_nodes);
         ...
         */
        rem_len = flat_ftype.lens[i] - avail_len;
                                   ...
                                   ADIO_Status *status, int *error_code)
{
    ...
    MPI_Aint lb;
    ADIOI_Offlen flat_ftype;
    ADIOI_Flatlist_node *flat_type = NULL;
    Flat_list flat_btype;
    ...
    flat_ftype.offsets = NULL;
    flat_ftype.lens = NULL;
    flat_ftype.count = 0;
    ...
    if (fd->hints->cb_write != ADIOI_HINT_DISABLE) {
         ...
         * lengths of write requests of this process into flat_ftype.offsets[]
         * and flat_ftype.lens[], respectively. The number of elements in both
         * flat_ftype.offsets[] and flat_ftype.lens[] is flat_ftype.count.
        ...
        ADIOI_Calc_my_off_len(fd, count, buftype, file_ptr_type, offset,
                              &flat_ftype.offsets, &flat_ftype.lens,
                              &start_offset, &end_offset, &flat_ftype.count);
        ...
        if (is_interleaved > 0) {
             ...
             */
            do_collect = ADIOI_LUSTRE_Docollect(fd, flat_ftype.count, flat_ftype.lens, nprocs);
        }
    }
    ...
    if ((fd->romio_write_aggmethod == 1) || (fd->romio_write_aggmethod == 2)) {
         ...
         */
        ADIOI_LUSTRE_IterateOneSided(fd, buf, flat_ftype.offsets,
                                     flat_ftype.lens, flat_ftype.count,
         ...
         */
        ADIOI_LUSTRE_Exch_and_write(fd, buf, &flat_btype, others_req,
                                    my_req, flat_ftype, min_st_loc,
                ...
                for (j = recv_curr_offlen_ptr[i]; j < others_req[i].count; j++) {
                    if (others_req[i].offsets[j] < iter_end_off) {
                        ...
                        others_req[i].mem_ptrs[j] =
                            (MPI_Aint) (others_req[i].offsets[j] - range_off);
...
if (probe_flag) {
printf("ERROR ++++ MPI_Iprobe rank=%4d is_agg=%d: ---- cb_nodes=%d ntimes=%d nbufs=%d\n",myrank,fd->is_agg,cb_nodes,ntimes,nbufs);
fflush(stdout);
}
...
#define CACHE_REQ(list, nelems, buf) {             \
    ...
    MPI_Get_address(buf, &list.disp[list.count]);  \
    list.count++;                                  \
}
...
static void ADIOI_LUSTRE_W_Exchange_data(
      ...
      const void                *buf,          /* user buffer */
            ...
            int                 *fileview_indx,
            ADIOI_Offlen        flat_ftype,    /* this process's file view offset-length pairs */
            ...
            int                 *error_code)   /* OUT: */
{
    ...
    *send_buf_ptr = NULL;
    ...
    if (flat_btype->count == 1) {
        ...
        ADIOI_LUSTRE_Fill_send_buffer(fd, buf, flat_btype, send_buf,
                                      fileview_indx, flat_ftype,
...
int num_memcpy=0;
    ...
    *self_buf = NULL;
     ...
     *
     * flat_btype stores the offset-length pairs of the flattened user buffer
     *     data type. Note this stores offset-length pairs of the data type,
     *     and write amount can be a multiple of the data type.
     * flat_btype->count: the number of pairs
     * flat_btype->off[i]: the ith pair's byte offset to buf. Note the
    ...
    user_buf_idx = flat_btype->extent * flat_btype->rnd
                 + flat_btype->off[flat_btype->idx]
                 + flat_btype->len[flat_btype->idx]
                 - flat_btype->rem;
     ...
     *     and flat_ftype.lens[i].
     * fileview_indx: the index to the flat_ftype.offs[], flat_ftype.lens[]
     ...
     */
    for (i = *fileview_indx; i < flat_ftype.count; i++) {
        off = flat_ftype.offsets[i];
        rem_len = flat_ftype.lens[i];
        ...
        while (rem_len != 0) {
            ...
            while (size) {
                int size_in_buf = MPL_MIN(size, flat_btype->rem);
...
num_memcpy++;
                    ...
                    /* update flat_btype->idx, flat_btype->rem,
                     * flat_btype->rnd, and user_buf_idx
                     */
                    if (flat_btype->idx < (flat_btype->count - 1))
                        flat_btype->idx++;
                    else {
                        flat_btype->idx = 0;
                        flat_btype->rnd++;
                    }
                    user_buf_idx = flat_btype->off[flat_btype->idx] +
                                   flat_btype->rnd * flat_btype->extent;
                    flat_btype->rem = flat_btype->len[flat_btype->idx];
...
num_memcpy++;
            ...
            off += len;
            rem_len -= len;
            flat_ftype.offsets[i] += len;
            flat_ftype.lens[i] -= len;
            send_total_size -= len;
            if (send_total_size == 0) goto done;
        }
    }
done:
    if (flat_ftype.lens[i] == 0) i++;
    *fileview_indx = i;

#ifdef WKL_DEBUG
if (num_memcpy> 0) printf("---- flat_ftype.count=%d i=%d flat_btype->rnd=%lld fileview_indx=%d flat_btype->count=%lld num_memcpy=%d\n",flat_ftype.count,i,flat_btype->rnd,*fileview_indx,flat_btype->count,num_memcpy);
#endif
}
                                         ...
                                         MPI_Datatype buftype, int myrank, int *error_code)
{
    ...
    while (doAggregation) {
        ...
        for (fileSegmentIter = 0; fileSegmentIter < numSegments; fileSegmentIter++) {
             ...
             */
            ADIOI_Flatlist_node *flat_type = ADIOI_Flatten_and_find(buftype);
            if (flat_type->count == 1) { /* buftype is contiguous */
